...
import { APIRoute, AppRoute, AuthorizationStatus, ERROR_401, NO_CONTENT_RESPONSE_CODE, SUCCESS_RESPONSE_CODE } from '../../const';
import { authorizationRequest, checkAuthRequest, loadUser, redirectToRoute, requireAuthorization, requireLogout } from '../actions';
...
describe('User async actions', () => {
  ...
  it('should dispatch AuthorizationRequest, LoadUser, RequriedAuthorization and RedirectToRoute when POST /login', async () => {
    ...
    mockAPI.onPost(APIRoute.Login).reply(SUCCESS_RESPONSE_CODE, {token: 'secret', ...user});
  ...
  it('should dispatch Logout when Delete /logout', async () => {
    mockAPI
      ...
      .reply(NO_CONTENT_RESPONSE_CODE);
