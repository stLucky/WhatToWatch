...
describe('Reducer: favorites', () => {
  ...
  it('should set value loading by a true', () => {
    expect(favorites(state, loadMyListRequest(true)))
      .toEqual({...state, isMyListLoading: true });
  });

  it('should set film will be downloaded from the server', () => {
    expect(favorites(state, loadMyListSuccess(fakeFilms)))
      .toEqual({...state, myList: fakeFilms });
  });

  it('should set value error by a true', () => {
    expect(favorites(state, loadMyListError(true)))
      .toEqual({...state, isMyListError: true });
  });
});
